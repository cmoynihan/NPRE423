...
class VoltageReader():
    """Docstring for PfiefferGaugeReader. """
    def __init__(self, root, pin, LJ=None, label='Voltage'):
        """TODO: to be defined. """
    ...
    def read(self):
        ...
        self.value = 100*V
        self.string_var.set(f'{self.value:.1f}')
        self.root.after(self.delay, self.read)

class CurrentReader():

    """Docstring for PfiefferGaugeReader. """
    def __init__(self, root, readers, resistance, label='Current'):
        """TODO: to be defined. """
    ...
    def read(self):
        ...
        :returns: TODO
        """
        V = abs(self.readers[1].value - self.readers[0].value)
        self.time=time.time()
        self.value = V/self.resistance
        self.string_var.set(f'{self.value:.3f}')
        self.root.after(self.delay, self.read)
...
class MainApp(tk.Tk):
    ...
    def __init__(self):
        ...
        plasmaV = VoltageReader(self, 'AIN0', LJ=self.LJ, label='Plasma Voltage')
        supplyV = VoltageReader(self, 'AIN1', LJ=self.LJ, label='Supply Voltage')
        current = CurrentReader(self, [plasmaV, supplyV], 201.6, label='Current')
        ...
        voltageframe = Info(self, label='Voltages', plottable=True, valuelabel='Volts')
        voltageframe.grid(row=1, column=0, sticky='news')
        voltageframe.addrow('Plasma Voltage', plasmaV)
        voltageframe.addrow('Supply Voltage', supplyV)

        currentframe = Info(self, label='Current', plottable=True, valuelabel='Amps')
        currentframe.grid(row=2, column=0, sticky='news')
        currentframe.addrow('Current', current)

        notebook = ttk.Notebook(self)
        notebook.grid(row=0, column=1, rowspan=4, sticky='news')

        plot = CanvasedPlot(self, [convectron],
                ['Convectron'],
                ylabel='Pressure (Torr)', notebook=notebook, tablabel='Pressure')
        plot.ax.set_yscale('log')

        Vplot = CanvasedPlot(self, [plasmaV, supplyV], ['Plasma Voltage', 'Supply Voltage'], ylabel='Voltage (V)', notebook=notebook, tablabel='Voltage')
        Iplot = CanvasedPlot(self, [current], ['Current'], ylabel='Current (A)', notebook=notebook, tablabel='Current')

        filewriter = FileWriter(self, [convectron])
        filewriter.controlframe.grid(row=3, column=0, sticky='news')
